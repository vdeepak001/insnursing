    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex items-center justify-between h-20 w-full" aria-label="Global">
            <a href="{{ route('home') }}" class="-m-1.5 p-1.5 flex items-center gap-2 group transition-transform hover:scale-105">
                <img src="{{ asset('IEB_original_logo.png') }}" alt="IHS Nursing Logo" class="h-16 w-auto object-contain">
            </a>
            ...
            <div class="flex lg:hidden">
                <button type="button" @click="mobileMenuOpen = true" class="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-slate-700 hover:text-impetus-orange transition-colors">
                ...
                @if (Route::has('login'))
                    @auth
                        <div class="flex items-center gap-3">
                            @if ($cartCount > 0)
                                <a href="{{ route('cart.index') }}" class="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-800 transition hover:border-impetus-orange hover:text-impetus-orange">
                            ...
                            @endif
                             ...
                             <div x-data="{ userMenuOpen: false }" class="relative flex-shrink-0 flex flex-col items-center gap-1.5">
                                ...
                                <div class="w-40 rounded-full bg-impetus-navy py-1 text-center text-[10px] font-bold tracking-wider text-white shadow-md font-outfit">
                                ...
                                >
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" class="block w-full rounded-xl px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-impetus-orange transition-colors font-outfit">
                                            Logout
                                        </button>
                                    </form>
                                </div>
                    ...
                    @else
                        <div class="flex items-center gap-3">
                            <button type="button" @click="$dispatch('open-login-modal')" class="text-sm font-bold text-slate-700 hover:text-impetus-orange transition-colors font-outfit">
                                Log in
                            </button>
                            <button type="button" @click="$dispatch('open-register-modal')" class="bg-impetus-orange hover:bg-[#e05d14] text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-impetus-orange/20 transition-all hover:scale-102 hover:shadow-xl hover:shadow-impetus-orange/30 font-outfit glow-orange">
                                Sign up
                            </button>
                        </div>
                    @endauth
        ...
        >
            <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-6 py-5">
                <a href="{{ route('home') }}" class="-m-1.5 flex items-center gap-2 p-1.5" @click="mobileMenuOpen = false">
                    <img src="{{ asset('IEB_original_logo.png') }}" alt="IHS Nursing Logo" class="h-10 w-auto object-contain">
                </a>
            ...
            <nav class="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-y-contain px-6 py-6" aria-label="Mobile">
                ...
                @if (Route::has('login'))
                    <div class="mt-8 border-t border-slate-200 pt-6">
                        @auth
                            @if (auth()->user()?->role_type === 'user')
                                <span class="-mx-3 block truncate rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 font-outfit">
                                    Hi, {{ auth()->user()->name }}
                                </span>
                                @if ($cartCount > 0)
                                    <a href="{{ route('cart.index') }}" class="-mx-3 flex items-center justify-between rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50 font-outfit font-semibold">
                                        <span>Cart</span>
                                ...
                                @endif
                            @else
                                <a href="{{ url('/dashboard') }}" class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50 font-outfit">Dashboard</a>
                            @endif
                            <a href="{{ route('profile') }}" class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50 font-outfit">
                                My Profile
                            </a>
                            <a href="{{ route('profile.change-password') }}" class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50 font-outfit">
                                Change Password
                            </a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="-mx-3 block w-full rounded-lg px-3 py-2.5 text-left text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50 font-outfit">
                                    Logout
                                </button>
                            </form>
                        @else
                            <button type="button" @click="mobileMenuOpen = false; $dispatch('open-login-modal')" class="-mx-3 block w-full rounded-lg px-3 py-2.5 text-left text-base font-bold leading-7 text-slate-900 hover:bg-slate-50 font-outfit">
                                Log in
                            </button>
                            <button type="button" @click="mobileMenuOpen = false; $dispatch('open-register-modal')" class="-mx-3 block w-full rounded-lg px-3 py-2.5 text-left text-base font-bold leading-7 text-slate-900 hover:bg-slate-50 font-outfit font-semibold">
                                Sign up
                            </button>
                        @endauth
                    </div>
