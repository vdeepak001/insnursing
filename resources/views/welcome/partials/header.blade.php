...
<div x-data="{ mobileMenuOpen: false, scrolled: false }" @keydown.escape.window="mobileMenuOpen = false"
    @scroll.window="scrolled = (window.pageYOffset > 50)" class="sticky top-0 z-50 w-full">
    ...
    <header class="w-full transition-all duration-300 glass-card border-b border-slate-200/50 shadow-sm">
        <div class="w-full px-4 sm:px-6 lg:px-10 xl:px-12 2xl:px-16">
            <nav class="grid h-20 w-full grid-cols-[1fr_auto] items-center xl:grid-cols-[auto_minmax(0,1fr)_auto] xl:gap-x-6"
                aria-label="Global">
                <a href="{{ route('home') }}"
                ...
                <!-- Desktop Menu -->
                <div
                    class="col-start-2 hidden min-w-0 items-center justify-evenly justify-self-stretch xl:flex xl:px-4 2xl:px-8">
                    ...
                    <button type="button" @click="$dispatch('open-contact-modal')"
                        class="shrink-0 whitespace-nowrap rounded-md px-1.5 py-1 text-xs 2xl:px-2 2xl:text-sm font-semibold text-slate-700 hover:text-impetus-teal transition-colors font-outfit cursor-pointer focus:outline-none">Contact
                        Us</button>
                </div>
                ...
                <!-- Desktop Actions -->
                <div
                    class="col-start-2 hidden items-center justify-self-end gap-2 2xl:gap-3 xl:col-start-3 xl:flex xl:justify-end">
                    @if (Route::has('login'))
                        @auth
                            <div class="flex items-center gap-3">
                                ...
                                <div x-data="{ userMenuOpen: false }"
                                    class="relative flex-shrink-0 flex flex-col items-center gap-1.5">
                                    ...
                                    <div class="w-40 rounded-full bg-impetus-teal py-1 text-center tracking-wider text-white shadow-md font-outfit"
                                    ...
                                    <div x-show="userMenuOpen" x-cloak x-transition
                                        ...
                                        <a href="{{ route('profile') }}" @click="userMenuOpen = false"
                                            class="block rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-impetus-teal transition-colors font-outfit">My
                                            Profile</a>
                                        <a href="{{ route('profile.change-password') }}" @click="userMenuOpen = false"
                                            class="block rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-impetus-teal transition-colors font-outfit">Change
                                            Password</a>
    ...
    <div x-show="mobileMenuOpen" x-cloak class="xl:hidden" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-[101] flex flex-col bg-white shadow-2xl"
            ...
            <div class="pointer-events-none absolute inset-0 opacity-[0.06]"
                style="background-image: radial-gradient(#045A5D 1.5px, transparent 1.5px); background-size: 28px 28px;">
            </div>

            {{-- Light header bar --}}
            <div
                class="relative z-10 flex shrink-0 items-center justify-between px-6 py-5 border-b border-slate-200/80 bg-white">
            ...
            {{-- Nav items --}}
            <nav class="relative z-10 flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-y-contain px-5 py-6"
                aria-label="Mobile">
                <div class="space-y-2">
                    ...
                    <button type="button" @click="mobileMenuOpen = false; $dispatch('open-contact-modal')"
                        class="block w-full rounded-xl px-4 py-4 text-left text-xl font-semibold leading-8 text-slate-700 hover:bg-slate-50 border-l-4 border-transparent hover:border-impetus-orange hover:text-impetus-orange transition-all font-outfit cursor-pointer focus:outline-none">Contact
                        Us</button>
                </div>
                ...
                @if (Route::has('login'))
                    <div class="mt-8 border-t border-slate-200 pt-6">
                        @auth
                            @if (auth()->user()?->role_type === 'user')
                                <span
                                    class="block truncate rounded-xl px-4 py-4 text-xl font-bold leading-8 text-slate-900 font-outfit">
                                    Hi, {{ auth()->user()->name }}
                                </span>
                                @if ($cartCount > 0)
                                    <a href="{{ route('cart.index') }}"
                            ...
                            <a href="{{ route('profile') }}"
                                class="block rounded-xl px-4 py-4 text-xl font-semibold leading-8 text-slate-900 hover:bg-slate-50 font-outfit">My
                                Profile</a>
                            <a href="{{ route('profile.change-password') }}"
                                class="block rounded-xl px-4 py-4 text-xl font-semibold leading-8 text-slate-900 hover:bg-slate-50 font-outfit">Change
                                Password</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
