@php
    ...
    $activeStates = \App\Models\State::query()->where('status', 'active')->orderBy('name')->pluck('name');
@endphp

<div x-data="{ open: {{ $frontendRegisterErrors->isNotEmpty() ? 'true' : 'false' }} }" x-show="open" x-cloak id="register-modal" role="dialog" aria-modal="true"
    aria-labelledby="register-modal-title" class="fixed inset-0 z-[9999] overflow-y-auto p-4 sm:p-6"
    @open-register-modal.window="open = true" @keydown.escape.window="open = false">
    ...
    <div class="relative z-10 w-full max-w-2xl mx-auto my-4 sm:my-8 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xl shadow-slate-900/10 sm:p-7"
        ...
        <div class="mb-5 flex items-start justify-between gap-4">
            <div>
                <h2 id="register-modal-title" class="font-serif text-xl font-semibold text-slate-900">Create account
                </h2>
                <p class="mt-1 text-sm text-slate-600">Register as a user with your nursing details. A random password
        ...
        @if (config('services.google.client_id'))
            <div class="mb-5">
                ...
                <p class="mt-2 text-center text-xs text-slate-500">Quick sign-in with Google. You can complete your
                    profile later from My Profile.</p>
            </div>
        ...
        <form method="POST" action="{{ route('frontend.register') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                <label for="register-name" class="mb-1.5 block text-sm font-medium text-slate-700">Full Name <span
                        class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
            ...
            <div>
                <label for="register-state" class="mb-1.5 block text-sm font-medium text-slate-700">State <span
                        class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
            ...
            <div>
                <label for="register-qualification"
                    class="mb-1.5 block text-sm font-medium text-slate-700">Qualification <span
                        class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
                <select id="register-qualification" name="qualification" required
                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-impetus-orange focus:outline-none focus:ring-2 focus:ring-impetus-orange/25">
            ...
            <div>
                <label for="register-dob" class="mb-1.5 block text-sm font-medium text-slate-700">Date of birth <span
                        class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
                <input type="date" id="register-dob" name="date_of_birth" value="{{ old('date_of_birth') }}" required
                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-impetus-orange focus:outline-none focus:ring-2 focus:ring-impetus-orange/25" />
                <x-input-error :messages="$frontendRegisterErrors->get('date_of_birth')" class="mt-2" />
            </div>

            <div>
                <label for="register-email" class="mb-1.5 block text-sm font-medium text-slate-700">Email <span
                        class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
            ...
            <div>
                <label for="register-phone" class="mb-1.5 block text-sm font-medium text-slate-700">Mobile Number
                    <span class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
                <input type="text" id="register-phone" name="phone" value="{{ old('phone') }}" required
                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-impetus-orange focus:outline-none focus:ring-2 focus:ring-impetus-orange/25" />
                <x-input-error :messages="$frontendRegisterErrors->get('phone')" class="mt-2" />
            </div>

            <div>
                <label for="register-rn-number" class="mb-1.5 block text-sm font-medium text-slate-700">RN Number
                    <span class="text-red-500 font-bold" style="color: #ef4444;">*</span></label>
                <input type="text" id="register-rn-number" name="rn_number" value="{{ old('rn_number') }}"
                    required
                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-impetus-orange focus:outline-none focus:ring-2 focus:ring-impetus-orange/25" />
            ...
            <div>
                ...
                <input type="text" id="register-uid" name="uid" value="{{ old('uid') }}"
                    oninput="this.value = this.value.replace(/[^a-zA-Z0-9]/g, '')" placeholder="Alphanumeric only"
                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-impetus-orange focus:outline-none focus:ring-2 focus:ring-impetus-orange/25" />
                <x-input-error :messages="$frontendRegisterErrors->get('uid')" class="mt-2" />
            </div>

            <div class="flex items-center pt-5 sm:pt-6">
                <span class="text-xs font-semibold text-red-600 sm:text-sm" style="color: #dc2626;">* Fields are
                    Mandatory</span>
            </div>
...
@if (session('registered_user'))
    <div x-data="{ open: true }" x-show="open" x-cloak id="register-success-modal" role="dialog"
        aria-modal="true" class="fixed inset-0 z-[9999] overflow-y-auto" @keydown.escape.window="open = false">
        <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity"
            x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-150"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="open = false"></div>
        ...
        <div class="relative z-10 w-full max-w-md mx-auto my-4 sm:my-8 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xl shadow-slate-900/10 sm:p-7"
            ...
            <div class="text-center">
                ...
                <p class="mt-2 text-sm text-slate-600">Your account has been created. Please keep your login details
                    safe.</p>
            </div>
            ...
            <div class="mt-6 rounded-xl bg-slate-50 p-4 border border-slate-200">
                <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 text-sm">
                    ...
                    <div>
                        <span
                            class="font-mono font-bold text-logo-blue bg-blue-50 px-2 py-1 rounded inline-block">{{ session('registered_user')['password'] }}</span>
                    </div>
                </div>
            </div>

            <div class="mt-6 flex flex-col gap-3">
                <button type="button" @click="open = false; $dispatch('open-login-modal')"
                    class="w-full rounded-full bg-impetus-orange px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-impetus-orange/90 focus:outline-none focus:ring-2 focus:ring-impetus-orange focus:ring-offset-2">
                    Proceed to Login
                </button>
            </div>
