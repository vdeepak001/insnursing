...
@section('content')
    <div x-data="orderDetails(@js(url(App\Helpers\MenuHelper::getCurrentPrefix() . '/users-list')), @js(csrf_token()))" @keydown.escape.window="if (courseOpen) { closeCourse() }">
            ...
            <div class="flex flex-wrap items-end gap-3">
                <div class="min-w-[220px] flex-1">
                    ...
                    <input id="order-search" name="search" type="text" value="{{ $filters['search'] }}"
                        placeholder="Name, IHS ID, email, module..." oninput="window.orderFiltersDebounceSubmit()"
                            ...
                            <tr class="odd:bg-white even:bg-impetus-teal-muted/50 hover:bg-brand-50/80 transition-colors">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-600 dark:text-gray-400 max-w-[12rem]">
                                    @if ($order->remarks)
                                        <span class="line-clamp-2"
                                            title="{{ e($order->remarks) }}">{{ $order->remarks }}</span>
                                    @else
                                        —
                                    @endif
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <x-ui.button type="button" variant="secondary" size="sm" class="!px-3 !py-2"
                                        @click="openCourse({{ $order->user_id }})" title="View purchased modules">
                                        View Report
                                    </x-ui.button>
                                </td>
                            </tr>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('orderDetails', (usersListBaseUrl, csrfToken) => ({
                ...
                async openCourse(userId) {
                    ...
                    try {
                        const res = await fetch(this.usersListBaseUrl + '/' + userId +
                            '/purchased-courses', {
                                headers: {
