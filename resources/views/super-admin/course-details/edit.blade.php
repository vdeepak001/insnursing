...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        ...
        <x-ui.button variant="outline" type="button"
            onclick="window.location='{{ route($routePrefix . '.course-details.index') }}'">
            Back to List
        </x-ui.button>
    </div>
        ...
        <x-common.component-card title="Course Information">
            <form method="POST" action="{{ route($routePrefix . '.course-details.update', $course) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg mb-6 border border-gray-200 dark:border-gray-700">
                        <!-- Course Name (Static) -->
                        <div>
                            <label
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="course_url" type="text" name="course_url"
                ...
                <div class="mt-6 grid grid-cols-12 gap-3 sm:gap-6 items-end">
                    <div class="col-span-6 min-w-0">
                            ...
                            <div class="mb-3 flex flex-wrap items-start gap-3 sm:gap-4">
                                @if ($course->attachmentIsImage())
                                    <a href="{{ $course->attachmentPublicUrl() }}" target="_blank" rel="noopener noreferrer"
                                        class="inline-block shrink-0" title="{{ basename($course->attachment) }}">
                                        <img src="{{ $course->attachmentPublicUrl() }}" alt="" loading="lazy"
                                ...
                                @else
                                    <a href="{{ $course->attachmentPublicUrl() }}" target="_blank"
                                        ...
                                        <svg class="mr-1.5 h-4 w-4 shrink-0" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                ...
                                @endif
                                <label
                                    class="inline-flex cursor-pointer items-center gap-2 text-xs sm:text-sm text-gray-700 dark:text-gray-400 shrink-0 pt-1">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Upload a new file to replace the current
                            attachment. PDF, Word, or image. Max 10 MB.</p>
                        @error('attachment')
                            <span class="text-red-600 text-sm mt-2">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-3 min-w-0">
                        ...
                        <input id="sequence" type="number" name="sequence"
                    ...
                    <div class="col-span-3 flex justify-end min-w-0">
                        <div x-data="{ switcherToggle: {{ $course->active_status == 1 ? 'true' : 'false' }} }">
                            ...
                            <label for="active_status"
                                class="flex cursor-pointer items-center gap-2 sm:gap-3 text-sm font-medium text-gray-700 select-none dark:text-gray-400 whitespace-nowrap">
                                <div class="relative">
                    ...
                    <div class="mt-8 border-t border-gray-200 pt-8 dark:border-gray-700">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input id="seo_key" type="text" name="seo_key"
                                    value="{{ old('seo_key', $course->seo_key) }}"
                                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                                @error('seo_key')
                                    <span class="text-red-600 text-sm mt-2">{{ $message }}</span>
                                @enderror
