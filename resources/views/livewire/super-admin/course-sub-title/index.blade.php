        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" wire:click="setFilter('active')" wire:key="filter-active" wire:loading.attr="disabled"
                        class="px-3 py-1.5 text-xs font-semibold rounded-lg transition-all {{ $filter === 'active' ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-200' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800' }}">
                    Active ({{ $activeCount }})
                </button>
                <button type="button" wire:click="setFilter('inactive')" wire:key="filter-inactive" wire:loading.attr="disabled"
                        class="px-3 py-1.5 text-xs font-semibold rounded-lg transition-all {{ $filter === 'inactive' ? 'bg-rose-600 text-white shadow-sm shadow-rose-200' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800' }}">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                @forelse ($subTitles as $title)
                    <tr wire:key="title-{{ $title->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400 align-top">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 align-top">
                            <div class="text-xs font-medium text-gray-900 dark:text-white">{{ $title->user?->name ?? 'System' }}</div>
                            <div class="text-[10px] text-gray-500 uppercase">{{ $title->user?->role_type ?? 'N/A' }}</div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-center align-top">
                            @if ($routePrefix === 'admin')
                                <button wire:click="toggleStatus({{ $title->id }})" class="focus:outline-none" wire:loading.attr="disabled">
                        ...
                        @if ($routePrefix === 'admin' && Route::has($routePrefix . '.course-sub-titles.edit'))
                            <td class="px-4 py-3 whitespace-nowrap text-right text-xs font-medium align-top">
                ...
                @empty
                    <tr>
                        <td colspan="{{ $routePrefix === 'admin' ? 6 : 5 }}" class="px-4 py-8 whitespace-nowrap text-sm text-gray-500 text-center dark:text-gray-400">
